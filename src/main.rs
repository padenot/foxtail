...
fn main() {
    ...
    if args.no_git { config.enable_git = false; }
    ...
    if let Err(e) = colors::validate_colors(&colors) { eprintln!("{}", e); std::process::exit(2); }
    let bg_rgb = match parse_color(&colors.background) {
        Some(rgb) => rgb,
        None => { eprintln!("Invalid background color in configuration"); std::process::exit(2); }
    };

    if data.cost.total_duration_ms / 1000 < 60
        && data.context_window.used_percentage > config.warn_high_initial_context_threshold
    {
        println!("{}", render_warning(&data, &config, bg_rgb, is_light_bg, color_enabled));
    } else {
        println!(
            "{}",
            render_formatted(
